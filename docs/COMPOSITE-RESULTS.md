...
adjustment is attempted. Partial upstream input, missing security dates,
ambiguous suspension boundaries, malformed prices, or inconsistent identity
become an integrity failure with `data: []` and failed completeness. Callers
must explicitly request raw-only output (`--no-adjusted` or `adjusted: false`)
to retain the ordinary partial-range behavior.
